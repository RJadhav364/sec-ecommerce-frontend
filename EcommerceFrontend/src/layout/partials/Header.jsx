...
const Header = ({ handleChild1Data }) => {
  ...
  return (
    <>
      <header className="border-b-[1px] border-[#c1c1c1] bg-white dark:bg-darkbg-default dark:border-[#423c3c] font-display-Montserrat sticky w-full top-0 z-[101]">
        <div className="mx-auto w-[1400px] max-[1534px]:w-full max-[1534px]:px-[20px] py-[20px] grid grid-cols-3 items-center">
          ...
          <div className="flex justify-end gap-[10px]">
            {isCustomerLogin ? (
              <div
                className="p-[6px_8px] flex flex-row items-center gap-[10px] cursor-pointer relative max-[769px]:hidden"
              ...
              >
                <div className="dark:hidden inline-block">
                  <img
                    src={person_dark}
                    alt=""
                    className="w-[40px] h-[40px] rounded-[50%] bg-[#E8E2E6]"
                  />
                </div>
                <div className="hidden dark:inline-block">
                  <img
                    src={person_white}
                    alt=""
                    className="w-[40px] h-[40px] rounded-[50%] bg-[#656565]"
                  />
                </div>
                <div className="flex flex-col gap-[5px]">
                  ...
                  <span className="dark:text-white text-[rgba(0,0,0,0.6)] text-[13px] font-[400] capitalize text-left justify-start">
                    {email}
                  </span>
                </div>
                {isOpen && (
                  ...
                  >
                    <div className="py-1 border-[#bab2b4]">
                      {dropDown &&
                        dropDown.map(
                          (
                            { label, img_dark, img_light, width, height, link },
                            index,
                          ) => (
                            <div key={index}>
                              <Link
                              ...
                              >
                                <img
                                  src={img_dark}
                                  className={`dark:hidden inline-block w-[30px] h-[30px]`}
                                  alt=""
                                />
                                <img
                                  src={img_light}
                                  className={`hidden dark:inline-block w-[30px] h-[30px]`}
                                  alt=""
                                />
                                {label}
                              </Link>
                    ...
                    <div className="py-1">
                      <div>
                        ...
                        >
                          <img
                            src={log_out_dark}
                            className={`dark:hidden inline-block w-[25px] h-[25px]`}
                            alt=""
                          />
                          <img
                            src={logout123_light}
                            className={`hidden dark:inline-block w-[25px] h-[25px]`}
                            alt=""
                          />
                          Logout
                        </a>
              ...
              <div className="flex gap-[8px] items-center max-[769px]:hidden">
                ...
                <span className="border-[1px] border-[#968287] h-[60%] inline-block"></span>
            ...
            <Link
              to={isCustomerLogin ? "customer/my-list" : "/login"}
              className="cursor-pointer hover:bg-[#e2dedf] p-[10px] rounded-full dark:text-white dark:hover:bg-[#444444] w-[50px] h-[50px] flex justify-center items-center relative max-[769px]:hidden"
            >
              <svg
                stroke="currentColor"
                fill="currentColor"
      ...
      ></div>
      <Relogin isReloginModelOpen={isReloginModelOpen} setIsReloginModelOpen={() => setIsReloginModelOpen(false)} />
    </>
  );
};
