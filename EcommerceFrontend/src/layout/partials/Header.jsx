...
const Header = () => {
  ...
  const {username , email , isCustomerLogin, wishList} = useCustomerStore();
    ...
    <div className='border-b-[1px] border-[#c1c1c1] bg-white dark:bg-darkbg-default dark:border-[#423c3c] font-display-Montserrat'>
      <div className='mx-auto w-[1400px] py-[20px] grid grid-cols-3 items-center'>
        ...
        <div className='flex justify-end gap-[10px]'>
          {
            isCustomerLogin ? (
              <div className='p-[6px_8px] flex flex-row items-center gap-[10px] cursor-pointer relative' onClick={toggleDropdown}>
                ...
                <div
                transition
                className={`absolute right-auto z-10 top-[55px] mt-2  origin-top-right divide-y divide-gray-100 rounded-md bg-white dark:bg-[#07070A] shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in ${isOpen ? "opacity-[100%] w-56 h-auto" : "opacity-0 w-0 h-0"}`}
                >
                  <div className="py-1 border-[#bab2b4]">
                    ...
                    {
                      dropDown && dropDown.map(({label,img_dark,img_light,width,height},index) => (
                        <div key={index}>
                          <a
                            href="#"
          ...
          <Link to={'/profile'} className='cursor-pointer hover:bg-[#e2dedf] p-[10px] rounded-full dark:text-white dark:hover:bg-[#444444] w-[50px] h-[50px] flex justify-center items-center relative'>
            ...
            <span className={`${isCustomerLogin ? "text-[13px] bg-[#ff5252] rounded-[50%] flex justify-center items-center absolute right-[8px] top-[24px] min-w-[19px]" : "hidden"}`}>{wishList}</span>
          </Link>
